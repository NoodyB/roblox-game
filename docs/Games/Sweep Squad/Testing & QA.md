...
| `logic.spec` (60) | Config integrity, schema migration and sanitizing, stats stacking, economy (buy, unlock, rebirth, perks, packs), DebrisGrid, ZoneLayout, NetCodec, critter odds, pity, golden and equip rules, quests and streaks, formatting, RNG, rate limiter, signals, promo codes, seasonal event windows, odds and drop share |
...
| `world.spec` (11) | Full map build against the Roblox API: gates, prompts, part budget (1,556), anchoring, leaderboards, spawn, sell-pad alignment with server logic; seasonal hub decor; every prop, critter (normal and golden, including event critters) and gear tier |
