...
- Store icons for all 10 passes and products. The owner created them on Roblox, and their IDs are wired into `Config/Products.luau`. The store test now checks that each button prompts its own ID with the right purchase type, and never prompts an owned pass. A catalogue test checks IDs are whole and unique. 123 tests pass.
