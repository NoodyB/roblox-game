...
- Monetization: 4 passes and 6 products with real IDs set (from the owner's dashboard). Idempotent receipts, save-before-grant. Purchase flow still needs a Studio test.
...
- Tooling: 123 tests, lint, format, place build and validator, economy simulator, geometry preview renderer and UI layout previews for phone, tablet and PC (`tools/preview`, `docs/previews/`), promo art including real-render thumbnails.
...
| Create badges; set the group ID (passes and products ✅ created, IDs wired in) | **You** | [Monetization](docs/Games/Sweep%20Squad/Monetization.md) checklist (the IDs go in `Config/Products`, `Config/Badges` and `GameConfig.GroupId`) |
...
- `./tools/check.sh` passes all steps: stylua ✅, selene 0/0 ✅, **123 passed / 0 failed** ✅, rojo build ✅, place validation ✅.
